    ...
    .get(async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                // execute the query to find that record
                const search = await connection.execute(`SELECT * FROM ${tablename} WHERE ${primaryKeyColumn} = ${recordId}`);
                const headers = search.metaData.map(column => column.name);
                const rows = search.rows;
                res.json({ headers, rows }); // send headers and rows of the record to the frontend.
